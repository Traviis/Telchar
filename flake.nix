          ...
          nixos-gate-3-contract =
            ...
            harness.mkTest {
              ...
              testScript = ''
                ...
                gateway.succeed("sudo -u postgres psql -d telchar-ingress -v ON_ERROR_STOP=1 -Atc \"SELECT count(*) FROM store_leases WHERE owner_kind = 'request' AND purpose = 'input' AND state = 'active'\" | grep -qx 1")
                gateway.succeed("sudo -u postgres psql -d telchar-ingress -v ON_ERROR_STOP=1 -Atc \"SELECT store_path FROM store_leases WHERE owner_kind = 'request' AND purpose = 'input' AND state = 'active'\" > /tmp/telchar-input-leases")
                gateway.succeed("test \"$(wc -l < /tmp/telchar-input-leases)\" -eq 1")
                gateway.succeed("leased_input=$(cat /tmp/telchar-input-leases); test -e \"$leased_input\"; grep -q telchar-source-input \"$leased_input\"")
