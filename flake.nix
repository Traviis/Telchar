          ...
          nixos-gate-3-contract =
            ...
            harness.mkTest {
              ...
              testScript = ''
                ...
                gateway.succeed("sudo -u postgres psql -d telchar-ingress -v ON_ERROR_STOP=1 -Atc \"SELECT count(*) FROM store_leases WHERE owner_kind = 'request' AND purpose = 'input' AND state = 'released' AND released_at IS NOT NULL\" | grep -qx 1")
                ...
                gateway.succeed("sudo -u postgres psql -d telchar-ingress -v ON_ERROR_STOP=1 -Atc \"SELECT count(*) FROM store_leases WHERE owner_kind = 'request' AND purpose = 'output' AND state = 'active' AND released_at IS NULL\" | grep -qx 1")
                ...
                gateway.succeed("test \"$(wc -l < /tmp/telchar-input-leases)\" -eq 1")
                gateway.succeed("released_input=$(cat /tmp/telchar-input-leases); test -e \"$released_input\"; grep -q telchar-source-input \"$released_input\"")
                gateway.succeed("output_root=$(find /var/lib/telchar-gc-roots -mindepth 1 -maxdepth 1 -type l -print); test \"$(printf '%s\\n' \"$output_root\" | sed '/^$/d' | wc -l)\" -eq 1; test \"$(readlink \"$output_root\")\" = \"" + output_path + "\"")
