...
use crate::store_export::{GatewayStoreExportBackend, NixStoreExportBackend, StoreExportBackend};
use crate::store_import::{GatewayStoreImport, StoreImportBackend};
use crate::store_query::GatewayStoreQuery;
use crate::store_retention::{
...
impl GatewayStoreRuntime {
    ...
    pub fn retention(&self) -> io::Result<Box<dyn StoreRetentionBackend>> {
        let Some(root_directory) = &self.gc_root_directory else {
            return Ok(crate::store_retention::unavailable_backend());
        };
    ...
    pub fn build_executor(&self) -> io::Result<Box<dyn BuildBackend>> {
        match &self.build_helper {
            Some(helper) => Ok(Box::new(crate::local_executor::NixStoreExecutor::new(
                helper,
                self.endpoint.to_string(),
            )?)),
            None => Ok(Box::new(crate::local_executor::GatewayStoreExecutor::new(
                self.endpoint.clone(),
            ))),
        }
    }
