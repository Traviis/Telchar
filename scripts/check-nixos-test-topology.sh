...
require "$contract" 'The client reaches the gateway only through the declared virtual network boundary.'
require "$contract" 'The packaged `telchar` system service owns service startup.'
require "$contract" 'Readiness requires the gateway service to be active and its health endpoint to answer over the virtual network.'
require "$contract" 'A protocol-compatible OTLP gRPC collector receives correlated startup logs, metrics, and traces.'
