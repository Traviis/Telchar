...
- One Telchar deployment serves exactly one configured Nix system and one bounded feature set. Different systems use independent daemon, PostgreSQL, gateway-store, and ingress deployments, exposed to clients as separate Nix builders.
...
A deployment rejects build requests whose system or required features do not match its configured envelope. Multi-architecture operation is composition: operators configure one builder URI per deployment and let Nix choose among them. Deployments for different systems do not share a Telchar database or gateway store in the initial architecture.
