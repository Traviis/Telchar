...
Telchar does not terminate TLS, redirect plaintext transport to TLS, infer a TLS requirement from Nomad configuration, or require TLS on a trusted network. Its callback listener accepts plaintext WebSocket. Operators expose `wss://` through a reverse proxy or load balancer that terminates TLS and forwards the WebSocket upgrade and exact subprotocol unchanged. The allocation worker validates the proxy's certificate when connecting to `wss://`. `ws://` provides no confidentiality; bearer tokens, store paths, derivation metadata, logs, and NAR contents may be visible to the trusted network. HMAC provides authentication and integrity over plaintext transport, not confidentiality. One transfer stays on one WebSocket connection, so load-balancer stickiness is unnecessary. Native TLS and certificate lifecycle management are explicit non-goals.
